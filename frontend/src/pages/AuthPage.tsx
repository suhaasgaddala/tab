...
function BrandPanel() {
  ...
  return (
    <div className="hidden lg:flex flex-col justify-between bg-zinc-950 px-12 py-12 relative overflow-hidden border-r border-zinc-800">
      <div className="tab-grid absolute inset-0 opacity-50" />
      <div className="absolute -top-40 -left-40 h-80 w-80 rounded-full bg-amber-500/6 blur-3xl" />
      <div className="absolute bottom-0 right-0 h-64 w-64 rounded-full bg-emerald-500/4 blur-3xl" />

      <div className="relative">
        <Link to="/" className="flex items-center gap-2.5">
          <span className="flex h-9 w-9 items-center justify-center rounded-lg bg-amber-500 font-mono text-sm font-black text-zinc-950 shadow-glow">
            T
          </span>
          <span className="text-lg font-bold text-zinc-50">Tab</span>
        </Link>
      </div>
      ...
      <div className="relative space-y-8">
        <div>
          ...
          <h2 className="text-3xl font-extrabold leading-tight tracking-tight text-zinc-50">
        ...
        <ul className="space-y-3">
          {points.map((pt) => (
            <li key={pt} className="flex items-center gap-3 text-sm text-zinc-300">
              <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs">
                ✓
              </span>
              {pt}
            </li>
      ...
      <p className="relative text-xs text-zinc-700 font-mono">© 2026 Tab · Built on x402</p>
    </div>
  );
}
...
function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  const inputClass =
    "w-full rounded-xl border border-zinc-700 bg-zinc-950 px-4 py-3 text-sm text-zinc-100 placeholder-zinc-600 outline-none transition-all focus:border-amber-500/50 focus:ring-2 focus:ring-amber-500/10";

  return (
    <div className="flex flex-col justify-center bg-zinc-900 px-6 py-12 sm:px-10 lg:px-16 xl:px-20">
      {/* Mobile logo */}
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="mt-8 space-y-4">
          {isSignup && (
            <div>
              <label className="mb-1.5 block text-xs font-semibold text-zinc-400">Company name</label>
          ...
          <div>
            <label className="mb-1.5 block text-xs font-semibold text-zinc-400">Work email</label>
          ...
          {!isSignup && (
            <div className="flex justify-end">
              <a href="#" className="text-xs font-semibold text-amber-500 hover:text-amber-400 transition-colors">
                Forgot password?
              </a>
            </div>
          )}
          ...
          <button type="submit" disabled={loading}
            className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-amber-500 py-3.5 text-sm font-bold text-zinc-950 shadow-glow transition-all hover:bg-amber-400 disabled:opacity-50 disabled:cursor-not-allowed">
            {loading ? (
              <>
        ...
        <button type="button"
          className="mt-4 flex w-full items-center justify-center gap-3 rounded-xl border border-zinc-700 bg-zinc-950 py-3 text-sm font-semibold text-zinc-300 transition-all hover:border-zinc-600 hover:bg-zinc-900">
